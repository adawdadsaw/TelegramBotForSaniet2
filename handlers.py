...
from config import PAYMENTS_TOKEN, item_url, item_url1, item_url2
...
async def info (message: Message):
    await message.answer('Здравствуйте, вас привествует магазин "JetStreetSam".Желаете приобрести игровую приставку Playstation 4?"!', reply_markup=keyboard)
...
@dp.message_handler(Text(equals=['PS 4']))
...
PRICES1 = [
    LabeledPrice(label='PS5', amount=2500000),
    LabeledPrice(label='Прочная упаковка', amount=20000)
]
...
@dp.message_handler(Text(equals=['PS 5']))
async def buy_process(message: Message):
    await bot.send_invoice(message.chat.id,
                           title=MESSAGES['item_title_1'],
                           description=MESSAGES['item_description_1'],
                           provider_token=PAYMENTS_TOKEN,
                           currency='rub',
                           photo_url=item_url1,
                           photo_height=512,
...
PRICES2 = [
    LabeledPrice(label='PS4 Pro', amount=1800000),
    LabeledPrice(label='Прочная упаковка', amount=20000)
]
...
@dp.message_handler(Text(equals=['PS 4 pro']))
async def buy_process(message: Message):
    await bot.send_invoice(message.chat.id,
                           title=MESSAGES['item_title_2'],
                           description=MESSAGES['item_description_2'],
                           provider_token=PAYMENTS_TOKEN,
                           currency='rub',
                           photo_url=item_url2,
                           photo_height=512,
